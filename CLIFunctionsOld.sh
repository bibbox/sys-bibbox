...
function bibbox-installApp() 
{
        ...
        #done
        declare name
        name=$(sudo python3 -c 'import sys; sys.path.insert(1, "/opt/bibbox/sys-bibbox/sys-backend"); import mainFunctions; x=mainFunctions.MainFunctions(); x.getAppName('"$var2"')' 2>&1)
        declare keylist
	declare paramlist
        params=$(curl https://raw.githubusercontent.com/bibbox/"${name}"/v8-4-x_tng/.env)
        #echo $appname
	sed -n s/' '/'='/g <<< $params
        echo https://raw.githubusercontent.com/bibbox/${name}/v8-4-x_tng/.env
...
#	echo $paramlist
        ...
        sudo python3 -c 'import sys; sys.path.insert(1, "/opt/bibbox/sys-bibbox/sys-backend"); import mainFunctions; x=mainFunctions.MainFunctions(); x.installApp('"$paramlist"','"$keylist"','"$var1"','"$var2"','"$var3"',CLI=True)'
