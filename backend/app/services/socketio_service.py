...
def emitInstanceRefresh():
    ...
    sio.emit('new_instance_data', {'data': 'New Instance Data in Backend'}, broadcast=True, namespace='/socket.io') #, namespace="/socket.io")
    sio.sleep(0)

def emitInstanceDeleted(instance_name):
    print('emitting instance delete info')
    sio = SocketIO(message_queue='redis://redis:6379')
    sio.emit('instance_deleted', {'id': instance_name}, broadcast=True, namespace='/socket.io') #, namespace="/socket.io")
    sio.sleep(0)